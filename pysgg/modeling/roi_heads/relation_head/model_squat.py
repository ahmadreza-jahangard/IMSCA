...
class MaskPredictor(nn.Module):
    def __init__(self, in_dim, h_dim,use_bert=False):
        super().__init__()
        self.h_dim = h_dim
    ...
    def forward(self, x,y=None):
        ...
        out = self.layer2(z)
        if self.use_bert:
            k = torch.cat((out,y.unsqueeze(1)),dim=-1)
            out = self.bert_layer(k)
...
class P2PDecoder(nn.Module):
    ...
    def forward(self, tgt, memory, ind, tgt_mask=None, memory_mask=None, 
                tgt_key_padding_mask=None, memory_key_padding_mask=None,embeddings=None):
        pair = tgt 
...
class P2PDecoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation=F.relu, norm_first=False,use_bert=False):
        super(P2PDecoderLayer, self).__init__() 
        self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        self.self_attn_node = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        self.self_attn_bert = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        ...
        self.use_bert = use_bert
        if self.use_bert:
            self.multihead_attn_b2b = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
            self.norm1_bert = nn.LayerNorm(d_model)
            self.dropout1_bert = nn.Dropout(dropout)
            self.dropout2_bert = nn.Dropout(dropout)
    ...
    def forward(self, tgt, memory, ind, tgt_mask=None, memory_mask=None, 
                tgt_key_padding_mask=None, memory_key_padding_mask=None,embedding=None):
        ...
        if embedding is not None:
            embedding = self.norm1_bert(embedding+ self._sa_bert_block(embedding,None,None))
            updated_pair = self.norm2(sparsified_pair + self._mha_e2e(sparsified_pair, pair_e2e, None, None) \
                                                  ...
                                                  + self._mha_e2b(sparsified_pair, embedding, None, None))
        else:
            updated_pair = self.norm2(sparsified_pair + self._mha_e2e(sparsified_pair, pair_e2e, None, None) \
                                                  + self._mha_e2n(sparsified_pair, sparsified_unary, None, None)) 
    ...
    def _sa_bert_block(self,x,attn_mask, key_padding_mask):
        x = self.self_attn_bert(x, x, x, attn_mask=attn_mask, 
                           key_padding_mask=key_padding_mask, 
                           need_weights=False)[0]
        return self.dropout1_bert(x)
    ...
    def _mha_e2b(self, x, mem, attn_mask, key_padding_mask):
        x = self.multihead_attn_e2n(x, mem, mem, 
                                      attn_mask=attn_mask, 
                                      key_padding_mask=key_padding_mask, 
                                      need_weights=False)[0]
        
        return self.dropout2_bert(x)
...
class SquatContext(nn.Module):
    def __init__(self, config, in_channels, hidden_dim=512, num_iter=3):
        ...
        self.use_bert_scores = self.cfg.MODEL.BERT.USE_BERT_SCORE
        self.use_bert_edge_embed = self.cfg.MODEL.BERT.USE_BERT_EDGE_EMBED
        ...
        if self.use_bert_edge_embed:
            self.bert_dim = self.cfg.MODEL.BERT.EMBEDDING_DIM
            self.b_embedding = nn.Sequential(
                nn.Conv1d(in_channels=3, out_channels=1, kernel_size=1),
                nn.Linear(self.bert_dim , self.hidden_dim*2),
                nn.ReLU(),
                nn.Linear(self.hidden_dim*2, self.hidden_dim*4),
                nn.ReLU()
            )
        norm_first = config.MODEL.ROI_RELATION_HEAD.SQUAT_MODULE.PRE_NORM
        decoder_layer = P2PDecoderLayer(self.pooling_dim, 8, self.hidden_dim * 2, norm_first=norm_first,use_bert=self.use_bert_edge_embed)
        ...
        self.mask_predictor = MaskPredictor(self.pooling_dim, self.hidden_dim,use_bert=self.use_bert_scores)
    ...
    def forward(self, roi_features, proposals, union_features, rel_pair_idxs, rel_gt_binarys=None, logger=None,bert_score=None,bert_embedding=None):
        ...
        if self.use_bert_edge_embed:
            bert_embedding = self.b_embedding(bert_embedding)
        ...
        masks = [self.mask_predictor(k,bert_score).squeeze(1) for k in feat_pred_batch_key] # num_rel X 1 
        ...
        if bert_embedding is not None:
            feat_pred_batch = [self.m2m_decoder(q.unsqueeze(1), (u.unsqueeze(1), p.unsqueeze(1)), (ind, ind_e2e, ind_n2e),embeddings=bert_embedding).squeeze(1) \
