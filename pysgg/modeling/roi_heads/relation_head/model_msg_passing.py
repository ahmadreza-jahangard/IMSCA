...
class PairwiseFeatureExtractor(nn.Module):
    ...
    def __init__(self, config, in_channels):
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
            ...
            self.mode = 'sgdet'
        self.use_bert_obj = self.cfg.MODEL.BERT.USE_OBJ
        ...
        if self.use_bert_obj:
            self.bert_obj_path = self.cfg.MODEL.BERT.OBJ_PATH
            self.bert_embeddings = load_bert_obj(self.bert_obj_path)
            self.bert_dim = self.cfg.MODEL.BERT.OBJ_DIM
        else:
            self.bert_dim = 0
        ...
        self.pairwise_obj_feat_updim_fc = make_fc(self.hidden_dim + self.obj_dim + self.embed_dim + self.bert_dim,
                                                  self.hidden_dim * 2)
        ...
        self.obj_feat_aug_finalize_fc = nn.Sequential(
            ...
            make_fc(self.hidden_dim + self.obj_dim + self.embed_dim+self.bert_dim, self.pooling_dim),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, inst_roi_feats, union_features, inst_proposals, rel_pair_idxs, ):
        ...
        if self.use_bert_obj:
            obj_embed_by_bert = self.bert_embeddings[obj_pred_labels,:]
        ...
        elif self.use_bert_obj:
            augment_obj_feat = cat((obj_embed_by_bert,inst_roi_feats, augment_obj_feat), -1)
        else:
