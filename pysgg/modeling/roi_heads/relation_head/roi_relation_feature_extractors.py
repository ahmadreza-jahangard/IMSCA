...
from pysgg.modeling.roi_heads.relation_head.model_msg_passing import load_bert_obj
...
class RelationFeatureExtractor(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        if self.geometry_feature:
            ...
            if self.use_bert_scores:
                self.bert_score_path = self.cfg.MODEL.BERT.SCORE_PATH
                self.bert_scores = load_bert_obj(self.bert_score_path)
            self.use_bert_edge_embed = self.cfg.MODEL.BERT.USE_BERT_EDGE_EMBED
            if self.use_bert_edge_embed:
                self.bert_edge_path = self.cfg.MODEL.BERT.EDGE_PATH
                self.bert_edge_embed = load_bert_obj(self.bert_edge_path)
    def forward(self, x, proposals, rel_pair_idxs=None):
        ...
        for proposal, rel_pair_idx in zip(proposals, rel_pair_idxs):
            ...
            if self.use_bert_scores or self.use_bert_edge_embed:
                labels = cat([each_prop.get_field("pred_labels") for each_prop in proposals], dim=0)
                head_labels = labels[rel_pair_idx[:, 0]]
                tail_labels = labels[rel_pair_idx[:, 1]]

                if self.use_bert_scores:
                    bert_score = self.bert_scores[head_labels,tail_labels]
                if self.use_bert_edge_embed:
                    bert_embed = self.bert_edge_embed[head_labels,tail_labels,:,:]
        ...
        return union_features,bert_score,bert_embed
